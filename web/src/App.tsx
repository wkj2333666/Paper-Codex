...
import {
  Activity, ArchiveRestore, BookOpen, Bot, CheckCircle2, ChevronDown, ChevronRight,
...
import { loadPanelVisibility, savePanelVisibility, setPanelOpen, type PanelName } from "./panel-preferences"
import type { Dashboard, GraphNode, GraphPayload, KnowledgeKind, Paper, PaperAnalysis, PaperDetail, Project, SearchResult } from "./types"

type Selection={kind:"workbench"|"inbox"|"paper"|"project"|"search"|"graph"|"trash";id?:string}
...
export default function App(){
  ...
  const [selection,setSelection]=useState<Selection>({kind:"workbench"})
  const [stream,dispatch]=useReducer(reduceEvent,initialState)
  const lastEvent=useRef(0)
  const [error,setError]=useState("")
  const [panels,setPanels]=useState(loadPanelVisibility)
  ...
  useEffect(()=>{setActiveDrawer(null)},[selection.kind,selection.id])
  ...
  return <div className={shellClass}>
    {(isNarrow||panels.sidebarOpen)
      ?<Sidebar dashboard={dashboard} selection={selection} select={setSelection} refresh={load} logout={logout} drawerOpen={activeDrawer==="sidebar"} onCollapse={()=>collapsePanel("sidebar")}/>
    ...
    <main className="main-pane">
      ...
      <MainView dashboard={dashboard} selection={selection} select={setSelection} refresh={load} paperGraphOpen={panels.paperGraphOpen} isNarrow={isNarrow} activeDrawer={activeDrawer} openPanel={openPanel} collapsePanel={collapsePanel}/>
    </main>
    {!graphMode&&((isNarrow||panels.codexOpen)
      ?<ActivityPanel state={stream} selection={selection} drawerOpen={activeDrawer==="codex"} onCollapse={()=>collapsePanel("codex")}/>
...
function Sidebar({dashboard,selection,select,refresh,logout,drawerOpen,onCollapse}:{dashboard:Dashboard;selection:Selection;select:Select;refresh:()=>Promise<void>;logout:()=>void;drawerOpen:boolean;onCollapse:()=>void}){
  ...
  return <aside className={`sidebar workspace-panel${drawerOpen?" drawer-open":""}`} data-panel="sidebar"><div className="brand"><div className="brand-icon"><BookOpen size={19}/></div><div><strong>Paper Codex</strong><span>论文研究记忆</span></div><PanelCollapseButton label="文件树" direction="left" onCollapse={onCollapse}/></div>
    ...
    <div className="sidebar-foot"><span>{dashboard.papers.length} 篇论文</span><button onClick={logout}><LogOut size={14}/>退出</button></div>
  </aside>
}
...
function MainView({dashboard,selection,select,refresh,paperGraphOpen,isNarrow,activeDrawer,openPanel,collapsePanel}:{dashboard:Dashboard;selection:Selection;select:Select;refresh:()=>Promise<void>;paperGraphOpen:boolean;isNarrow:boolean;activeDrawer:PanelName|null;openPanel:(panel:PanelName,trigger:HTMLButtonElement)=>void;collapsePanel:(panel:PanelName)=>void}){
  if(selection.kind==="paper"&&selection.id)return <PaperView id={selection.id} dashboard={dashboard} select={select} refresh={refresh} paperGraphOpen={paperGraphOpen} isNarrow={isNarrow} drawerOpen={activeDrawer==="paperGraph"} onExpandGraph={trigger=>openPanel("paperGraph",trigger)} onCollapseGraph={()=>collapsePanel("paperGraph")}/>
  ...
  if(selection.kind==="graph")return <GraphWorkspace dashboard={dashboard} focusNode={selection.id} select={select}/>
  if(selection.kind==="trash")return <TrashView select={select} refresh={refresh}/>
  return <Workbench dashboard={dashboard} select={select}/>
}

function Workbench({dashboard,select}:{dashboard:Dashboard;select:Select}){
  ...
  const submit=async(event:FormEvent)=>{event.preventDefault();if(!source.trim())return;setBusy(true);try{await api.intake(source,project||undefined);setSource("")}finally{setBusy(false)}}
  const upload=async(file?:File)=>{if(!file)return;setBusy(true);try{await api.upload(file,project||undefined)}finally{setBusy(false)}}
...
function PaperView({id,dashboard,select,refresh,paperGraphOpen,isNarrow,drawerOpen,onExpandGraph,onCollapseGraph}:{id:string;dashboard:Dashboard;select:Select;refresh:()=>Promise<void>;paperGraphOpen:boolean;isNarrow:boolean;drawerOpen:boolean;onExpandGraph:(trigger:HTMLButtonElement)=>void;onCollapseGraph:()=>void}){
  const [detail,setDetail]=useState<PaperDetail|null>(null);const [graph,setGraph]=useState<GraphPayload>({nodes:[],edges:[]});const [project,setProject]=useState("");const [tab,setTab]=useState("overview");const [reanalyzing,setReanalyzing]=useState(false)
  const reload=useCallback(async()=>{const [paper,graph]=await Promise.all([api.paper(id),api.graph({paper_id:id})]);setDetail(paper);setGraph(graph)},[id])
  ...
  const openPdf=async()=>{const blob=await api.pdf(id);window.open(URL.createObjectURL(blob),"_blank","noopener")}
  ...
  return <div className={`paper-page${!isNarrow&&!paperGraphOpen?" paper-graph-collapsed":""}`}><div className="paper-reading"><header className="paper-head"><div><p className="eyebrow">{detail.paper.year??"论文"} · {detail.paper.doi??detail.paper.arxiv_id??detail.paper.id}</p><h1>{detail.paper.title}</h1><p>{authors.join(", ")||"作者信息待补充"}</p></div><div className="paper-head-actions"><button className="outline" onClick={()=>void openPdf()}><FileText/>打开原文</button><button className="outline" disabled={reanalyzing} onClick={()=>void reanalyze()}>{reanalyzing?<LoaderCircle className="spin"/>:<Sparkles/>}重新分析</button><button className="danger-outline" onClick={()=>void trash()}><Trash2/>移入回收站</button></div></header>
    ...
    <div className="reading-tabs">{[["overview","概要"],["method","方法与实验"],["results","结果"],["limitations","局限与复现"],["evidence","证据"]].map(([key,label])=><button key={key} className={tab===key?"active":""} onClick={()=>setTab(key)}>{label}</button>)}</div><AnalysisTab tab={tab} analysis={detail.analysis}/>
  </div>{!isNarrow&&!paperGraphOpen
    ?<PanelRail panel="paperGraph" label="相关知识" side="right" className="paper-graph-rail" onExpand={onExpandGraph}/>
    :<aside className={`paper-graph workspace-panel${drawerOpen?" drawer-open":""}`} data-panel="paperGraph"><div className="paper-graph-head"><div><p className="eyebrow">相关知识</p><h2>这篇论文连接了什么？</h2><p>实线表示有论文证据，弱化关系表示 Codex 的待验证假设。</p></div><PanelCollapseButton label="相关知识" direction="right" onCollapse={onCollapseGraph}/></div><GraphErrorBoundary><Suspense fallback={<GraphLoading/>}><SemanticGraph compact payload={graph} focusNode={id} onOpenFull={()=>select({kind:"graph",id})}/></Suspense></GraphErrorBoundary></aside>}</div>
}
...
function GraphWorkspace({dashboard,focusNode,select}:{dashboard:Dashboard;focusNode?:string;select:Select}){
  ...
  return <div className="graph-workspace"><aside className="graph-filters"><p className="eyebrow">研究关系网络</p><h1>知识图谱</h1><p>按项目查看论文、方法、概念、数据集与研究发现之间的联系。</p><label>项目范围<select value={project} onChange={event=>setProject(event.target.value)}><option value="">全部论文</option>{dashboard.projects.map(item=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label><fieldset><legend>节点类型</legend>{allKinds.map(kind=><label key={kind}><input type="checkbox" checked={kinds.has(kind)} onChange={()=>toggle(kind)}/><span className={`kind-dot ${kind}`}/>{kindName(kind)}<em>{counts[kind]??0}</em></label>)}</fieldset><label className="hypothesis-toggle"><input type="checkbox" checked={hypotheses} onChange={event=>setHypotheses(event.target.checked)}/>显示 Codex 假设关系</label><div className="edge-key"><span><i className="edge-solid"/>证据支持</span><span><i className="edge-dashed"/>待验证假设</span></div></aside>
    <section className="graph-stage">{busy?<GraphLoading/>:<GraphErrorBoundary><Suspense fallback={<GraphLoading/>}><SemanticGraph payload={graph} focusNode={focusNode} onSelectionChange={setSelected} onPaperOpen={paperId=>select({kind:"paper",id:paperId})}/></Suspense></GraphErrorBoundary>}</section>
...
function ActivityPanel({state,selection,drawerOpen,onCollapse}:{state:typeof initialState;selection:Selection;drawerOpen:boolean;onCollapse:()=>void}){const [question,setQuestion]=useState("");const [busy,setBusy]=useState(false);const ask=async(event:FormEvent)=>{event.preventDefault();if(!question.trim())return;setBusy(true);try{await api.question(selection.kind==="paper"?"paper":selection.kind==="project"?"project":"global",selection.id??null,question);setQuestion("")}finally{setBusy(false)}};return <aside className={`activity-pane workspace-panel${drawerOpen?" drawer-open":""}`} data-panel="codex"><div className="activity-title"><div><Bot size={18}/><strong>Codex 活动</strong></div><span className="live-dot">实时</span><PanelCollapseButton label="Codex" direction="right" onCollapse={onCollapse}/></div><div className="activity-feed">{state.activities.length?state.activities.map(item=><div className="activity-item" key={item.id}><div className={item.type==="failed"?"event-dot failed":item.type==="result"?"event-dot done":"event-dot"}>{item.type==="failed"?<CircleAlert/>:item.type==="result"?<CheckCircle2/>:<Activity/>}</div><div><p>{item.label}</p><span>{item.taskId.slice(0,8)} · {item.createdAt?new Date(item.createdAt).toLocaleTimeString():"刚刚"}</span></div></div>):<div className="quiet"><Bot/><p>Codex 正在等待任务</p></div>}</div>{state.latestAnswer&&<div className="answer-card"><p className="eyebrow">最近回答</p><ReactMarkdown remarkPlugins={[remarkGfm]}>{state.latestAnswer}</ReactMarkdown></div>}<form className="chat-box" onSubmit={ask}><textarea value={question} onChange={event=>setQuestion(event.target.value)} placeholder={selection.kind==="paper"?"询问这篇论文…":selection.kind==="project"?"询问这个项目…":"询问整个论文库…"}/><button disabled={busy||!question.trim()}>{busy?<LoaderCircle className="spin"/>:<Send/>}</button></form></aside>}
