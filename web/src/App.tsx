...
export default function App(){
  ...
  const researchChanged=useCallback((projectId:string)=>{
    setResearchRevisions(current=>({...current,[projectId]:(current[projectId]??0)+1}))
    void load()
  },[load])
  ...
  return <div className={shellClass} style={shellStyle}>
    ...
    <main className="main-pane">
      ...
      <MainView dashboard={dashboard} selection={selection} select={select} refresh={load} researchRevisions={researchRevisions} citationOverlay={citationOverlay} citationFocus={citationFocus} candidateFocus={candidateFocus} onCandidateFocusHandled={()=>setCandidateFocus(null)} paperGraphOpen={panels.paperGraphOpen} paperGraphWidth={panels.widths.paperGraph} isNarrow={isNarrow} activeDrawer={activeDrawer} openPanel={openPanel} collapsePanel={collapsePanel} resizePanel={resize} resetPanel={reset} theme={resolvedTheme}/>
    </main>
...
function ProjectView({project,dashboard,select,refresh,researchRevision,candidateFocus,onCandidateFocusHandled,theme}:{project?:Project;dashboard:Dashboard;select:Select;refresh:()=>Promise<void>;researchRevision:number;candidateFocus:CandidateFocus|null;onCandidateFocusHandled:()=>void;theme:ResolvedTheme}){
  ...
  const breadcrumb=projectBreadcrumb(dashboard.projects,project.id)
  const inherited=inheritedProjectPapers(dashboard.projects,dashboard.project_memberships,dashboard.papers,project.id)
  ...
  return <div className="content-wrap"><nav className="project-breadcrumb" aria-label="项目层级">{breadcrumb.map((item,index)=><span key={item.id}>{index>0&&<ChevronRight/>}<button aria-current={item.id===project.id?"page":undefined} onClick={()=>select({kind:"project",id:item.id})}>{item.name}</button></span>)}</nav><header className="project-hero"><div className="folder-large"><Folder/></div><div><p className="eyebrow">研究项目</p><h1>{project.name}</h1><p>{project.purpose||"尚未写下研究目标"}</p></div><div className="project-actions"><button onClick={()=>void rename()}><Pencil/>重命名</button><button onClick={()=>void remove(false)}><Trash2/>删除并上移子项目</button>{children.length>0&&<button className="danger" onClick={()=>void remove(true)}><Trash2/>删除整个子树</button>}</div></header><div className="project-meta"><span>{papers.length} 篇直接论文</span><span>{children.length} 个子项目</span><span>论文可被多个项目引用</span></div>{inherited.length>0&&<section className="inherited-project-context"><header><div><h2>继承上下文</h2><p>父项目论文可供阅读和 Codex 引用，但不属于当前项目。</p></div></header><div className="inherited-project-groups">{inherited.map(group=><section className="inherited-project-group" key={group.project.id}><button onClick={()=>select({kind:"project",id:group.project.id})}><Folder/>{group.project.name}</button><div className="inherited-paper-list">{group.papers.map(paper=><button key={paper.id} onClick={()=>select({kind:"paper",id:paper.id,projectId:project.id})}><BookOpen/>{paper.title}</button>)}</div></section>)}</div></section>}<ProjectResearch project={project} papers={papers} theme={theme} researchRevision={researchRevision} focusWorkId={candidateFocus?.workId} onFocusHandled={onCandidateFocusHandled} onOpenPaper={paperId=>select({kind:"paper",id:paperId})} onRemovePaper={removePaper} onChanged={refresh}/></div>
}
