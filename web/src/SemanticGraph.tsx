...
import { buildGraph, kindColor, kindLabel } from "./graph-model"
...
function Loader({payload,compact}:{payload:GraphPayload;compact:boolean}){
...
export function SemanticGraph({payload,compact=false,focusNode,onOpenFull,onPaperOpen,onSelectionChange}:Props){
  ...
  const settings=useMemo(()=>({
    ...
    defaultNodeColor:"#738078",
    defaultEdgeColor:"rgba(70,78,69,.45)",
    nodeReducer:(node:string,data:Record<string,unknown>)=>{
      if(neighborhood&&!neighborhood.has(node))return {...data,color:"#d8d8d0",label:""}
  ...
  }),[compact,edgeEndpoints,neighborhood,selected])
  ...
  return <div className={compact?"semantic-graph compact":"semantic-graph"}>
    ...
    <SigmaContainer className="sigma-canvas" settings={settings}>
      <Loader payload={payload} compact={compact}/><Events setHovered={setHovered} select={select}/><Controls/>
    </SigmaContainer>
    {payload.nodes.length===0&&<div className="graph-empty">尚无图谱数据。Codex 完成论文分析后会自动建立连接。</div>}
    {!compact&&<div className="graph-legend">{(["paper","concept","method","dataset","finding"] as KnowledgeKind[]).map(kind=><span key={kind}><i style={{background:kindColor(kind)}}/>{kindLabel(kind)}</span>)}<span><i className="edge-solid"/>有证据</span><span><i className="edge-dashed"/>假设</span></div>}
