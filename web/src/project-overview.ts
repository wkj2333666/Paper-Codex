...
export interface ProjectOverviewWorkItem {
  id:string
  kind:"goal"|"import"|"candidate"|"search"
  title:string
  detail:string
}
...
export function buildProjectOverview(input:ProjectOverviewInput){
  ...
  const importantWork:ProjectOverviewWorkItem[]=[]
  for(const goal of input.goals.filter(item=>item.status==="active"))importantWork.push({id:`goal:${goal.conversation_id}`,kind:"goal",title:goal.objective,detail:`Goal · ${goal.conversation_title}`})
  for(const candidate of activeCandidates.filter(item=>item.status==="importing"))importantWork.push({id:`import:${candidate.work.id}`,kind:"import",title:candidate.work.title,detail:"正在导入、评阅并写入知识图谱"})
  for(const candidate of activeCandidates.filter(item=>item.status==="candidate").slice(0,4))importantWork.push({id:`candidate:${candidate.work.id}`,kind:"candidate",title:candidate.work.title,detail:candidate.relevance_reason})
  for(const search of input.searches.filter(item=>item.state==="running"||item.state==="failed").slice(0,2))importantWork.push({id:`search:${search.id}`,kind:"search",title:search.question,detail:search.state==="running"?"检索正在进行":"检索需要处理"})
  ...
  const keyPapers=input.papers.map(paper=>({id:paper.id,title:paper.title,year:paper.year,connections:degrees.get(paper.id)??degrees.get(`paper:${paper.id}`)??0})).sort((left,right)=>right.connections-left.connections||left.title.localeCompare(right.title)).slice(0,6)
