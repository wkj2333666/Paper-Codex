  ...
  return <div className="conversation-history">
    ...
    <div className="conversation-list">
      ...
      {items.map(item=><article className={`conversation-row${item.id===activeConversationId?" active":""}`} key={item.id}>
        {view==="active"
          ?<button type="button" className="conversation-row-main" onClick={()=>onOpen(item.id)}><strong>{item.title}</strong><span>{updatedAt(item)}</span></button>
          :<div className="conversation-row-main"><strong>{item.title}</strong><span>{updatedAt(item)}</span></div>}
