...
async fn cancellation_is_a_terminal_compare_and_set_transition() {
    let db = Database::connect("sqlite::memory:").await.unwrap();
    let id = db.create_task("ingest", r#"{"source":"paper"}"#).await.unwrap();

    assert!(db.cancel_task(&id).await.unwrap());
    assert!(!db.cancel_task(&id).await.unwrap());
    assert!(db.transition_task(&id, TaskState::Resolving, None).await.is_err());
...
async fn candidate_import_task_completion_and_failure_update_project_state() {
    ...
    db.insert_paper("doi:10.1000/completed", "Completed")
        ...
        .unwrap();
    store
        .formalize_project_paper(&completed_task, "doi:10.1000/completed", &project)
        .await
        .unwrap();
    store
